...
import { Plus, Loader2, Library, Code2, BookOpen, Newspaper, Filter, X } from 'lucide-react';
  ...
  const fetchPosts = async () => {
    try {
      ...
      setPosts(data.map(post => ({
        ...post,
        technologies: post.tech_feed_post_technologies.map((pt: any) => pt.technologies),
        info_blocks: post.info_blocks.map((block: any) => ({
          ...block,
          items: block.items.sort((a: any, b: any) => a.order_index - b.order_index)
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Tech Feed</h1>
        <button
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Post Types</h3>
        ...
        {/* Technologies */}
        <div className="space-y-2">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">Technologies</h3>
          <div className="flex flex-wrap gap-2">
            ...
            {availableTechs
              .filter(tech => !selectedTechs.some(t => t.name === tech.name))
              .map(tech => (
                <button
                  key={tech.name}
                  onClick={() => handleTechToggle(tech)}
